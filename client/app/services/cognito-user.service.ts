...
export class CognitoUserService {
        ...
        onSuccess: (result) => {
          ...
          (<AWS.CognitoIdentityCredentials>creds).refresh((err) => {
            if (err) {
              ...
              resolve(this.setUserAttributes(cognitoUser));
              // resolve({
        ...
        if (err) {
          ...
          (<AWS.CognitoIdentityCredentials>creds).refresh((err) => {
            if (err) {
              ...
              const currentUser = this.setUserAttributes(cognitoUser);
              AWS.config.credentials = creds;
              resolve(creds);
            }
          });
  ...
  setUserAttributes(cognitoUser): Promise<any> {
    const self = this;
    return new Promise((resolve, reject) => {
      cognitoUser.getUserAttributes(function(err, result) {
        ...
        // console.log('  - CognitoUserService.setUserAttributes AWS_ENV.s3',  AWS_ENV.s3);
        ...
        // globals.status.currentUser = self.currentUser;
